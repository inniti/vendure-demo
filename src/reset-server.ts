...
import { RouteService } from "./plugins/routes/services/route.service";
...
function populateServer() {
  ...
  return populate(
    ...
    .then((app) => createTestRoutes(app).then(() => app))
    .then((app) => app.close());
}
...
async function createTestRoutes(app: INestApplication) {
  const routeService = app.get(RouteService);
  const requestContextService = app.get(RequestContextService);
  const ctx = await requestContextService.create({ apiType: "admin" });

  await routeService.create(ctx, {
    path: "/test",
    entity: "product",
    entityId: 1,
  });
